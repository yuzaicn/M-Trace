...
- 每个身份一轮：36 次调用，输出约 27,000 token，输入约 3,000 token；乘 1.5 重试系数，再做两轮（建库 + 2 周漂移复测）。
- 采集前另预留一次 16 整数最小挑战试打；models.dev 的价格只是预算依据，实际金额以官方账单为准。
...
| 型号 ID               | 发布       | 输入 / 输出 USD/MTok | 单轮 CNY | ID/探活状态                       |
| --------------------- | ---------- | -------------------: | -------: | --------------------------------- |
...
库内单轮小计：Qwen ¥5.76，Gemini ¥4.77，合计 **¥10.53**。
...
按 owner 新口径：12 模型 × 1 轮 ¥10.53；×1.5 重试系数 ¥15.79；×2 轮（建库 + 漂移复测）¥31.59；库外 20 身份（每身份 9 次调用）¥4.39；试打余量 ¥15.00；预算合计约 **¥50.98（≈¥51）**，低于 ¥1200 硬顶，未削减模型数或采样量。
