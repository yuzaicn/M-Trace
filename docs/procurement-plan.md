...
owner 随后将 429 改为无限自动等待类：优先使用 `retry-after`，缺失时按 60 秒起步指数退避、10 分钟封顶；每次等待前后写 raw checkpoint，429 不消耗有限重试或成功计费预算。首个自动续采运行从 `gpt-5.6-luna` 开始，首条 challenge 连续收到 5 次 429，依次完成 60 / 120 / 240 / 480 秒等待，实际累计 900 秒；第 5 次 600 秒等待前 checkpoint 写入后按单次运行时长边界退出。该型号仍为 0/36、缺口 36，新增成功计费 ¥0；下次运行先恢复未完成的 600 秒等待，再继续便宜型号，最后处理 Pro 的 13 个缺口。
