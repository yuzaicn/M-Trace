...
单轮金额按每模型约 3,000 输入 + 27,000 输出 token 的 models.dev 估算口径，仅用于比较；reasoning token 与重试上浮尚未实测。

| 官方 `model` 字段 | 发布       | 归因世代         | 输入/输出 USD/MTok | 最低 effort |       基础单轮 CNY | ID/试点状态                                         |
| ----------------- | ---------- | ---------------- | -----------------: | ----------- | -----------------: | --------------------------------------------------- |
...
- 基础测算约 ¥58/轮。reasoning 隐藏 token 和重试上浮后，暂估 ¥90–120/轮；两轮 + 库外 + 试点预期总额 **¥200–300**，硬顶仍为 **¥1200**。
