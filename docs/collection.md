...
`m-trace-collect` supports OpenAI-compatible `/v1/chat/completions` and Anthropic `/v1/messages`, including SSE and JSON responses. Rate-limit HTTP 429 responses are an unlimited-wait class: the collector honors `retry-after` when present, otherwise uses exponential waits clamped to 60 seconds through 10 minutes, and writes before/after retry checkpoints with cumulative wait. A 429 explicitly classified as `insufficient_quota` or `credit_balance_exhausted` instead writes a terminal checkpoint and exits for account funding; empty or unclassified 429 responses retain the rate-limit behavior. HTTP 5xx and timeout failures retain bounded retry/backoff, while other 4xx responses stop immediately. The collector enforces a per-request timeout, applies a request-per-minute interval, and resumes by skipping challenge IDs already present in the normalized JSONL file.
...
Explicit HTTP 4xx responses other than 429 are never retried. Rate-limit and unclassified HTTP 429 responses retry without consuming the bounded retry budget or successful-spend budget; each wait writes a `retry-checkpoint-v1` before and after sleeping, so a long run can exit and resume cleanly from the raw checkpoint. A quota-exhausted 429 writes only the sanitized error `type` and `code` in a `quota-exhausted` checkpoint, clears any pending wait on resume, and exits without trying another challenge. Network/timeout failures and HTTP 5xx retain the configured bounded retry/backoff policy. A transport tunnel is acceptable only when TLS still terminates at `api.openai.com`; invoke Node with `--use-env-proxy`, set `HTTPS_PROXY` to the reviewed CONNECT endpoint, and declare `--transport tunnel`. The CLI rejects tunnel provenance when either proxy precondition is absent. `--transport` records the actual route; it does not itself create a tunnel.
